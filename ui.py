...
class GolfUI:
    ...
    def draw_board(self, env):
        ...
        # Hole (Just take first one since they are all same)
        hx, hy = env.hole_pos[0][0] * WIDTH, env.hole_pos[0][1] * HEIGHT
        r = env.hole_radius * WIDTH
        pygame.draw.circle(self.screen, (50, 50, 50), (int(hx), int(hy)), int(r) + 2) # Rim
        pygame.draw.circle(self.screen, HOLE_COLOR, (int(hx), int(hy)), int(r))
        
    def draw_balls(self, env):
        # Only draw the FIRST active ball for clean visualization
        # We process 100 in background, but user only sees the "Leader"
        
        # Check if first ball is active or succeeded
        ...
        # 3. Shading (Gradient-like via multiple circles)
        # Darker bottom-right
    ...
    def draw_trail(self, trail, color=(255, 255, 255), width=2):
        # Trail only for the FIRST ball to avoid chaos
        if len(trail) < 2:
            return
